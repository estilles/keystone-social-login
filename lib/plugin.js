...
var plugin = function plugin(list) {
	...
	if (!_.isObject(list)) {
		this.__error('User "list" argument must be an object', 'ConfigError');
	}
	...
	_.each(this._options.providers, function(options, provider) {
		var providerLoginId = provider + 'LoginId';

		// provider specific keystone field
		keystoneFields[providerLoginId] = { type: String, label: options.loginId.label, width: 'medium', initial: true, index: true };
		...
		list.schema.pre('save', function(next) {
			var profileID = 'social.' + provider + '.id';
			...
			if (!this.isNew) {
				...
				if (this.isModified(providerLoginId) && (this.get(providerLoginId) !== this.get(profileID))) {
					this.set('social.' + provider + '.id', '');
					this.set('social.' + provider + '.name', '');
					this.set('social.' + provider + '.email', '');
					this.set('social.' + provider + '.photoUrl', '');
					this.set('social.' + provider + '.profileUrl', '');
				}
			}
